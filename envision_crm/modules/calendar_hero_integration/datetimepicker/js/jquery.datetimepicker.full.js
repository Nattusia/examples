			...
			if (!$(this).hasClass('xdsoft_scroller_box')) {
				...
				timeboxparent
					.on('scroll_element.xdsoft_scroller', function (event, percentage) {
						...
						var timesLength = options.allowTimes.length;
						var height = timesLength * options.timeHeightInTimePicker;
						if (height == 0) {
						  timeboxTop = parseFloat(Math.abs((timebox[0].offsetHeight - parentHeight) * percentage).toFixed(4));
					  }
					  else {
              timeboxTop = parseFloat(Math.abs((height - parentHeight) * percentage).toFixed(4));
            }
				...
				.on('xchange.xdsoft', function (event) {
					...
					xchangeTimer = setTimeout(function () {
						...
						line_time = function line_time(h, m) {
							...
							var timeFormatted = options.hours12 ?
                 now.toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true }) :
							   dateHelper.formatDate(now, options.formatTime);
							time += '<div class="xdsoft_time ' + classes.join(' ') + '" data-hour="' + h + '" data-minute="' + m + '">' + timeFormatted  + '</div>';
						};
				...
				.on('afterOpen.xdsoft', function () {
					if (options.timepicker) {
						...
						if (classType) {
							...
							var timesLength = options.allowTimes.length;
							if (timesLength > 0) {
							  height = timesLength * options.timeHeightInTimePicker;
              }
							top = timebox.find(classType).index() * options.timeHeightInTimePicker + 1;
//							top = (timebox.find(classType).index() + 1) * options.timeHeightInTimePicker;
							if ((height - pheight) < top) {
								top = height - pheight;
							}
							timeboxparent.trigger('scroll_element.xdsoft_scroller', [parseInt(top, 10)]);
